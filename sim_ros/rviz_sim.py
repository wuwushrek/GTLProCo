...
def sendBins():
	...
	for (key, binsVal) in binsDict.items():
		# if key in [7,8,11,16]: # Obstacles gridworld
		#   continue
		...
		for elem in binsVal:
			# Gazebo
			marker.scale.x = elem[2]
			marker.scale.y = elem[3]
			marker.pose.position = Point(elem[0],elem[1],-agentRadius)
	...
	# marker.scale.x = binUnitDim * 1.5
	# marker.scale.y = binUnitDim * 1.5
	marker.scale.x = binUnitDim * 15
	marker.scale.y = binUnitDim * 15
	for key, binsVal in binsDict.items():
		ranPos = random.choice(binsVal)
		marker.pose.position = Point(ranPos[0],ranPos[1],0)
		marker.text = str(key) # "Bin " + str(key)
		marker.id = nbElement
		marker.header.seq = nbElement
		marker.header.stamp = rospy.Time.now()
		nbElement = nbElement + 1
...
def sendObstacles():
	...
	for elem in obstaclesList:
		marker.scale.x = (elem[1][0]-elem[0][0] )
		marker.scale.y = (elem[2][1]-elem[1][1] )
...
if __name__ == '__main__':
	...
	parser.add_argument('--bins_name', type=str, default='world_25_bins.json')
	parser.add_argument('--obstacle_name', type=str, default='collision_final.npy')
	parser.add_argument('--swarm_traj', type=str, default='agent_50_time_10.npy')
	parser.add_argument('--update_rate', type=int, default=100)
	parser.add_argument('--agent_radius', type=float, default=1)
	parser.add_argument('--bin_unit_dim', type=float, default=5)
	parser.add_argument('--vmax', type=float, default=1.6)
	...
	robotPub = rospy.Publisher('/robot_position', Marker, queue_size=50)
	...
	# Sanity check
	assert 2*agentRadius < binUnitDim, 'Agent size must be at least twice than the unit size of a cell'
	...
	sim = rvo2.PyRVOSimulator(stepTime, 8.0*agentRadius, 6, 10*stepTime, 10*stepTime,
	...
	rC = [1,0,0]
	bC = [0,0,1]
	cellsColor = [np.random.random(3) for i in range(nBins)]
	...
	# ############## Robots in the gazebo simulation #################
	...
	# robotYaw = [0.0 for i in range(nbAgents)]
	...
	#     nextV, rGoal = updateVelocity(robotPosition, targetPos, robotVelocity)
